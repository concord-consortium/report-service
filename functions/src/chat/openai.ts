...
// per-page parent doc, and only the NEW message is sent as `input` each turn (no replay). The page
// system prompt is installed ONCE as a persistent developer-role conversation ITEM (NOT via
// `instructions`, which is per-request and not carried across turns). Assistant replies are structured:
// text.format json_schema/strict with a nullable `userText`; the model returns userText:null for
// routine observations. Verified end-to-end against the real API (openai@6.45, gpt-5.5).
...
// forwarded log telemetry (same role family as the persistent page-prompt item).
...
// the model CHOOSING userText:null for routine logs is driven by the CHAT_GENERIC_PROMPT rule.
...
// Install the page system prompt once as a persistent developer-role conversation item. It
// auto-carries-forward to every later turn, so subsequent turns send only the new user/log message.
