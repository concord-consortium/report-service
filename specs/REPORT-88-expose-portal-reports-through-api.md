...
- **Timeout / pool.** An overall wall-clock deadline is tracked in the chunk loop, and the same budget is the enclosing transaction's checkout deadline. Those are the only two bounds: a MyXQL cursor fetch ignores a `:timeout` option and reads from the socket with no deadline of its own, so there is no per-batch fetch timeout to set (established in REPORT-128, which carries the source references and the measurements). The per-server pool (`pool_size: 5`) is shared with auth/authz/bulk/web; the concurrency cap keeps downloads below it. The held read view (inherent to `MyXQL.stream`, which must run in a transaction) is an accepted risk bounded by the timeout.
...
- **Empty allowed-projects scoping (shared fix).** A project-admin/researcher with zero allowed projects made `apply_allowed_project_ids_filter` render `project_id IN ()` (a MySQL syntax error), pre-existing and identical in the web LiveView download. Refactored into `scope_by_allowed_projects/5`: `:all` -> no scoping, non-empty list -> scope as before, empty list / `:none` -> constrain to zero rows (`1 = 0`), and a `{:error, _}` permission-lookup failure -> raise `AllowedProjectsLookupError` (post-review correction: a failed lookup must stay visible, not become a believable empty result; download-build rescue -> clean 500, bulk/LiveView -> visible error, matching REPORT-76). A no-projects owner now gets a clean header-only `200`, mirroring the bulk empty-permission short-circuit; also corrects the web UI.
...
**Context**: Streaming holds a portal-DB connection and an open read view for the whole download, exposure the web UI does not have (it materializes before pushing bytes). The transaction's checkout deadline is what bounds the download; `MyXQL.stream`'s `:timeout` bounds nothing, because a cursor fetch discards it.
**Options considered**: keep the 5-minute query timeout; enforce an API-specific configurable timeout.
**Decision**: Enforce an API-specific configurable download timeout, via `PORTAL_DOWNLOAD_TIMEOUT_MS` (default 120000): an overall wall-clock deadline checked in the chunk loop, and the same budget as the transaction's checkout deadline. Pre-deadline with no bytes sent -> clean JSON error; mid-stream -> aborted stream. Independent of pool sizing (out of scope).
