...
- `PortalKeys` logs and ignores an entry it cannot trust: a missing field, an unreadable PEM, or a `kid` listed twice, where it trusts no entry for that `kid`, counting repeats before validating any entry, because which issuer the `kid` is bound to would be a guess. Unset or malformed, it trusts no key and every assertion is refused.
...
- The mint endpoint stores a role flag as true only when its claim is literally `true` (not `!!`). It requires `login`, `first_name`, `last_name` and `email` as non-empty strings and `portal_user_id` as a positive integer equal to the assertion's `uid`, answering 400 naming the claim. The fields are required because `create_user` would store a missing field as NULL and `update_user`'s changeset would fail on it. The `uid` check is there because `uid` is the researcher `/run-package` checked the assertion names. A `jti` over 255 characters, or an `exp` too far out to record, is refused as not authenticated, and `used_portal_assertions` also has an index on `expires_at` for the prune.
...
- `launching_until` is epoch milliseconds, cleared with `null`, and the launch records are written in a transaction rather than a batch, since the `Db` slice the module uses has no batch. The claim is cleared only when neither a VM nor a late mint can exist: a `GetMicrovmImage` failure, report-server refusing the mint or returning no token, a definite 4xx from `RunMicrovm`, or the oversized payload. The claim is left to lapse after any of these: the mint failing in transit (report-server may still mint, and a late mint would revoke the token a next launch hands its VM), a timeout or 5xx from `RunMicrovm`, or a failure to record a launched VM.
- Each upstream call is made once with a 10-second timeout (the mint including its body, and the SDK client with `maxAttempts: 1` and `throwOnRequestTimeout`), so the four calls a launch makes fit inside the function's 60 seconds. A throttled call is answered 502 with the work kept rather than retried. The timeout is a `setTimeout` race that aborts the mint's request, body read included, through an `AbortController` where one exists. `AbortSignal.timeout()` is not used because `ensure-vm.test.ts` runs in Jest 24's jsdom environment, whose `AbortSignal` has no `timeout()`, and the node environment has no `AbortSignal` at all.
...
Checks on the head commit: report-server 1065 tests pass (7 skipped) and compiles with `--warnings-as-errors`; the functions' 612 tests pass (32 suites, 8 emulator tests skipped), with `tsc` and `tslint` clean.
