...
A write the server never answered exits 1 (`INTERNAL`) rather than 6 (`TRANSIENT`), and that is a decision rather than an accident of `TransientError` never being reached on a non-idempotent request. Exit 6 tells a caller that retrying is the right move, which is true of a read and exactly wrong here: if the request did reach the server the run exists, so a wrapper that retries on 6 creates a second run, a second portal learner query and a second S3 upload. The unknown outcome is carried by the envelope's `action` field, which `EmitError` writes to stdout with the rest of the error, so a script can branch on it without the exit code being overloaded. `AsWriteCLIError`'s test asserts the classification so the tidier-looking 6 cannot be introduced later without the reason surfacing.
