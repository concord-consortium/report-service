...
- **report-server's stack.** cloud-formation's `fargate/report-server.yml` does not yet carry `PACKAGE_BUCKETS`, `PACKAGES_AWS_ACCESS_KEY_ID`, `PACKAGES_AWS_SECRET_ACCESS_KEY`, `PACKAGES_CORS_ORIGINS` or `PACKAGES_UNREVIEWED_RUNS`, nor REPORT-141's `PORTAL_PUBLIC_KEYS`. It lives outside this repository, and RD-1's first pass owns it (widened 2026-09-24, Doug). Unset, report-server boots and the catalog answers, but no portal can publish and no launch token verifies.
