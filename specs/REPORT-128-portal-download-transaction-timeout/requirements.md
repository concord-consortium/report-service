...
**The download's first-byte idle period equals the query time.** The header is not chunked until the first batch arrives (`report_controller.ex:295`), and an aggregate's first fetch runs the whole query, so a `school-metrics` download sends nothing for the length of its query and then everything. Anything between the client and the server that bounds idle time on a response therefore sees the full query duration as silence. Measured against production on 2026-09-09, first byte against total: run 206 (state CA) 29.96 s of 30.17 s, and run 201 (unfiltered) 70.71 s of 71.55 s. The silence is 99 per cent of the download in both cases, so a 60-second idle timeout anywhere in front of the server would cut the unfiltered download after the query had already been spent.

**Checked, and the edge is not a constraint.** Nothing in this repository configures that layer, so it was read from the deployed stacks. Both `report-service-prod` (account 612297603577) and `report-service-qa` (816253370536) put the service behind a shared `fargate-public-cluster` ALB, contributing only listener rules and a target group to it, and both load balancers report `idle_timeout.timeout_seconds` of **600**: `app/farga-Publi-1R328TZK4E1PC` in production and `app/farga-Publi-VIQR22PP0CN3` in QA. That is ten times the AWS default and five times the download budget, so `portal_download_timeout_ms` always cuts first and no download this endpoint can produce reaches the idle timeout.

The 600 is declared rather than ambient, which is the better reason to trust it: it is `idle_timeout.timeout_seconds` on `PublicLoadBalancer` in the sibling `cloud-formation` repo, `fargate/public-network-stack.yml:163-169`, and `fargate/report-server.yml` reaches that load balancer by importing from the `fargate-public-network` stack it names at `:8`. So the value is reviewable and greppable, and a change to it goes through a pull request there rather than happening quietly in the console. What it is not is this service's to set: the same load balancer fronts everything else in the cluster, so the invariant worth keeping is that 600 stays above `portal_download_timeout_ms`.
...
It is `PORTAL_DOWNLOAD_TIMEOUT_MS` (`runtime.exs:61-63`), so an operator can move it without a deploy if a slow run turns up, and this story's job is to make the configured value reachable rather than to re-choose it. Now measured against production rather than assumed: run 206 takes 30.2 s and an unfiltered `school-metrics` run takes 71.5 s for 4,051 rows. The headroom is therefore about 1.7 times, not the wide margin this section first claimed on run 206 alone. 120,000 ms still covers the heaviest run anyone has produced, and it stays the default, but the margin is thin enough that a slower portal or a larger corpus reaches it, and the operator lever is a config change rather than a deploy. The exposure is bounded by `max_concurrent: 2` against `pool_size: 5`, and the buffered web path already holds a connection for up to five minutes.
...
- **Not verifiable against the fixture database**: that `school-metrics` and `summary-metrics-by-subject-area` complete once the budget is raised. The fixture has no data that would exercise either report, so this needs the production portal. It was subsequently run there on 2026-09-09 and the results are in `implementation.md`'s Verification section: run 206 at 30.2 s, run 200 at 7.6 s, run 201 at 71.5 s, all 200s, and run 206's CSV byte-identical to the buffered path the web page uses.
