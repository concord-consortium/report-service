...
It is `PORTAL_DOWNLOAD_TIMEOUT_MS` (`runtime.exs:61-63`), so an operator can move it without a deploy if a slow run turns up, and this story's job is to make the configured value reachable rather than to re-choose it. Now measured against production rather than assumed: run 206 takes 30.2 s and an unfiltered `school-metrics` run takes 71.4 s for 4,051 rows. The headroom is therefore about 1.7 times, not the wide margin this section first claimed on run 206 alone. 120,000 ms still covers the heaviest run anyone has produced, and it stays the default, but the margin is thin enough that a slower portal or a larger corpus reaches it, and the operator lever is a config change rather than a deploy. The exposure is bounded by `max_concurrent: 2` against `pool_size: 5`, and the buffered web path already holds a connection for up to five minutes.
