...
The one thing to know about that number is that it is not this stack's to rely on: the ALB is shared with everything else in the `fargate-public-cluster`, so another service lowering it would silently shorten every Portal download. If that is ever a concern, the target to watch is 600 seconds staying above `portal_download_timeout_ms`.
