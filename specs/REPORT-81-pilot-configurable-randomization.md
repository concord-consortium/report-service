...
- **A classifier maps the origin class word prefix to a program**: `ft-` to full-time, `fl-` to flex, matched against the normalized (lowercased) form the portal stores.
...
- ⚠️ **The de-duplication fixes the arm, not the destination, and one same-arm double-enrolment path stays open.** `destinationClassWord` is re-derived on every run from the origin class word resolved *that run*, so a student whose origin class changes between two clicks keeps their arm and receives the new section's or teacher's destination word. **Both are in the same arm**, so no arm is contaminated and study validity is intact; this is a roster-tidiness problem rather than a data-integrity problem, which is why it is accepted and stated rather than fixed.
...
- **CI.** A `test_functions` job runs the lint pass, the unit suite and the emulator suite on every push touching `functions/**`. Before it, no CI job ran this package at all, so the import constraint above rested on an unenforced test. The job deliberately does **not** run `npm run build`: `tsc` emits compiled copies of every test into `lib/`, and four of them fail on the same `firebase-admin/auth` resolution problem; `lib/` is gitignored, so a CI checkout never has it.
...
- **Four pre-launch checks on `FALL_PRE_TEST` remain open.** None is a code dependency, and all four bear on an object that ships as a copy of spring's wording pinned by a deep-equality test. If any comes back different, the correct response is one change that edits the object, updates its attribution comment and deletes the pin.
...
**Context**: The de-duplication walk makes the **arm** sticky, but `destinationClassWord` is re-derived on every run from the origin class word resolved that run. A student whose origin class changes between two clicks therefore keeps their arm (correct) and receives a different destination word, so if the first click's enrolment had succeeded and the run then failed before `lock-activity`, they end up enrolled in two destination classes.
**Options considered**:
- A) Accept and document it.
- B) Persist the destination word beside the arm and reuse it.

**Decision**: **A.** Severity is genuinely lower than the arm case, and that is the point of stating it: both destinations are in the **same arm**, so no arm is contaminated and the study's validity is intact. It is a roster-tidiness problem for a teacher rather than a data-integrity problem for the PI. B was rejected because it would put a second value in the assignment document and make a legitimate mid-study class change unfollowable.
