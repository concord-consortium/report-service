...
- **The stale `pilot: "fall-2026-fulltime"` in `harness/im-done-local/run-step.js` is not cleaned up.** It is inert (the driver calls one compiled step directly and `pilot` reaches only the mint's `description` string). Cleanup belongs with REPORT-82. *(Corrected after REPORT-82 ran the grep: this sentence originally called it "the only fall pilot string in the tree", and it was not. The value appeared in **four** places, three of them shipped unit tests and one of those an assertion rather than a fixture. REPORT-82 replaced all four.)*
