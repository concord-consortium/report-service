...
Track C's `tool_id` is selected but unused: BR3's fold into the column key is deferred, and an XR4 assertion checks it is ignored rather than folded. `document_history_id` is **not** passed to the history link verbatim: the literal `"first"` is treated as absent (VR24).
...
So build the link with `maybe_document_history_id: nil` when the value is `"first"`. `HistoryLink.format_link_to_work/1` already omits the parameter for nil (`history_link.ex:23`), which means no history request is made, the playback controls stay closed, and the document opens normally: an honest "here is the document" rather than a false position. The report cannot do better, because the true first entry's id is not in the log.
