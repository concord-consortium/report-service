...
const environmentCredentials = {
  qa: {
    AWS_ACCESS_KEY_ID: process.env.QA_AWS_ACCESS_KEY_ID,
    AWS_SECRET_ACCESS_KEY: process.env.QA_AWS_SECRET_ACCESS_KEY,
  },
  production: {
...
if (args.length > 0) {
  ...
  if (/^\d{4}-\d{2}-\d{2}$/.test(ymd)) {
    if (isNaN((new Date(ymd)).getTime())) {
      console.error(`Invalid date provided: ${ymd}`);
      process.exit(1);
    }
  } else {
...
(async () => {
  for (const environment of environments) {
    const env = environmentCredentials[environment];
    if (!env.AWS_ACCESS_KEY_ID || !env.AWS_SECRET_ACCESS_KEY) {
      console.error(`Missing AWS credentials for environment: ${environment}`);
      continue;
    }
    ...
    const log = (message) => console.log(`${ymd} (${environment}): ${message}`);
    ...
    if (steps.includes("upload")) {
      ...
      execSync(command, { cwd: mergeRoot, env, stdio: 'inherit' });
    }
