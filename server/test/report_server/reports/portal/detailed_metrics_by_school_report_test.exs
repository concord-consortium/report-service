defmodule ReportServer.Reports.Portal.DetailedMetricsBySchoolReportTest do
  use ExUnit.Case, async: true
  alias ReportServer.Reports.{ReportFilter, ReportQuery}
  alias ReportServer.Reports.Portal.DetailedMetricsBySchoolReport
  ...
  describe "get_query/2 basic structure" do
    test "constructs basic query with no filters" do
      report_filter = %ReportFilter{
        country: [1],  # Need at least one filter to avoid "no filters" error
        state: nil,
        subject_area: nil,
      ...
      assert String.contains?(sql, "FROM portal_schools ps")
      assert String.contains?(sql, "GROUP BY ps.id")
      assert String.contains?(sql, "ORDER BY school_name asc")

      # Verify all expected columns are present
      assert String.contains?(sql, "trim(ps.name) AS school_name")
      assert String.contains?(sql, "trim(pd.name) AS school_district")
      assert String.contains?(sql, "trim(ps.city) AS school_city")
      assert String.contains?(sql, "coalesce(ps.state, '(Unknown)') AS school_state")
      assert String.contains?(sql, "coalesce(pc.name, '(Unknown)') AS school_country")
      ...
      assert String.contains?(sql, "left join portal_countries pc on (pc.id = ps.country_id)")
      assert String.contains?(sql, "left join portal_districts pd on (pd.id = ps.district_id)")
      assert String.contains?(sql, "join portal_school_memberships psm")
  ...
  describe "apply_filters/3 - country filter" do
    test "applies single country filter" do
      report_filter = %ReportFilter{
        country: [1],
        state: nil,
        subject_area: nil,
      ...
      assert String.contains?(sql, "WHERE (ps.country_id IN (1))")
    end
  ...
  describe "apply_filters/3 - state filter" do
    test "applies single state filter" do
      report_filter = %ReportFilter{
        country: nil,
        state: ["CA"],
        subject_area: nil,
  ...
  describe "apply_filters/3 - date filters" do
    test "applies start_date filter" do
      report_filter = %ReportFilter{
        country: [1],
        state: nil,
        subject_area: nil,
