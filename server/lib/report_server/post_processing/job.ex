defmodule ReportServer.PostProcessing.Job do
  ...
  defmodule JobOverrides do
    defstruct input_preprocess: nil, input_process: nil, output: nil, get_learners: nil
  end
  ...
  defp preprocess_rows(job, query_result, overrides) do
    ...
    if length(actions) > 0 do
      ...
      case get_input_stream(query_result, overrides.input_preprocess) do
        {:ok, stream } ->
          preprocessed = stream
          |> CSV.decode()
  ...
  defp process_rows(job, query_result, job_server_pid, preprocessed, overrides) do
    # since reports can be huge we need to stream them and decode them line by line into rows
    case get_input_stream(query_result, overrides.input_process) do
      {:ok, stream } ->
        s3_url = stream
        |> CSV.decode()
  ...
  defp get_input_stream(query_result, stream) do
    if stream do
      {:ok, stream}
    else
  ...
  defp get_learners(%{portal_url: portal_url}, run_remote_endpoints, overrides) do
    if overrides.get_learners do
      overrides.get_learners.()
    else
