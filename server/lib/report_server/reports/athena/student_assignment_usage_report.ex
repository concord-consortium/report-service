defmodule ReportServer.Reports.Athena.StudentAssignmentUsageReport do
  use ReportServer.Reports.Report, type: :athena, api_report_type: :usage
  ...
  def get_query(report_filter = %ReportFilter{}, user = %User{portal_server: portal_server}) do
    ...
    with {:ok, learner_data} <- LearnerData.fetch_and_upload(report_filter, user),
         ...
         {:ok, query} <- SharedQueries.get_usage_or_answers_athena_query(:usage, report_filter, resource_data, portal_server) do
      {:ok, query}
    else
      error -> error
    end
