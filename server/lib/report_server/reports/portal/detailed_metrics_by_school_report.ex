defmodule ReportServer.Reports.Portal.DetailedMetricsBySchoolReport do
  use ReportServer.Reports.Report, type: :portal

  def get_query(report_filter = %ReportFilter{}, user = %User{}) do
    %ReportQuery{
      cols: [
        {"trim(ps.name)", "school_name"},
        {"trim(pd.name)", "school_district"},
        {"trim(ps.city)", "school_city"},
        {"coalesce(ps.state, '(Unknown)')", "school_state"},
        {"coalesce(pc.name, '(Unknown)')", "school_country"},
        ...
        {"group_concat(distinct at.tag order by at.tag separator ', ')", "subject_areas"}
      ],
      ...
      join: [[
        ...
        "left join taggings t on (t.taggable_type = 'ExternalActivity' and t.taggable_id = ea.id and t.context = 'subject_areas')",
        "left join admin_tags at on (at.id = t.tag_id and at.scope = 'subject_areas')",
  ...
  defp apply_filters(report_query = %ReportQuery{},
    ...
    {join, where} = if have_filter?(subject_area) do
      {
        join, # use existing taggings/admin_tags joins
        ["at.id IN #{list_to_in(subject_area)}" | where]
      }
    else
      {join, where}
    end
